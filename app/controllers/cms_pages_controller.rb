class CmsPagesController < ApplicationController
	...
	def website_structure
		if @website = current_user.website
			...
			@independent_pages = @website.pages.where("navbar_item_id is NULL").order(:created_at)
		end
	end
	...
	def website_preview
		...
		@navbar_items = @website.navbar_items.order(:created_at)
		@independent_pages = @website.pages.where(["navbar_item_id is NULL and website_id = ?", @website.id]).order(:created_at)
	end
	...
	def show_page
		...
		@navbar_items = @website.navbar_items.order(:created_at)
		@independent_pages = @website.pages.where(["navbar_item_id is NULL and website_id = ?", @website.id]).order(:created_at)
		@page = Page.where("website_id = ? and slug = ?", @website.id, params[:page_slug]).first 
	end
